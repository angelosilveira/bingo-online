...
import { getConferenciaByBingoId } from "@/services/bingoGestaoService";
import { ConferenciaItem } from "@/models/BingoGestao";

interface BingoConferenciaProps {
  bingoId: string;
  quantidadeCartelas: number;
}

const BingoConferencia = ({ bingoId, quantidadeCartelas }: BingoConferenciaProps) => {
  ...
  useEffect(() => {
    const loadConferencia = async () => {
      try {
        const conferenciaData = await getConferenciaByBingoId(bingoId);
        setConferencia(conferenciaData);
        setNumerosSorteados(conferenciaData.map(item => item.numero));
      } catch (error) {
...
export default BingoConferencia;
